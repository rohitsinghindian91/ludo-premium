...
class _LoginPageState extends State<LoginPage> {
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF0A0E1A), body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
      ...
      const SizedBox(height: 12),
      TextField(controller: referCtrl, style: const TextStyle(color: Colors.white), decoration: InputDecoration(labelText: "Referral Code (Optional)", filled: true, fillColor: Color(0xFF151A2B), border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide.none))),
      const SizedBox(height: 24),
      SizedBox(width: double.infinity, height: 54, child: ElevatedButton(onPressed: goOtp, style: ElevatedButton.styleFrom(backgroundColor: Colors.amber, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))), child: const Text("LOGIN / GET OTP 1234", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900))))
    ]))));
  }
}

class OtpPage extends StatefulWidget {
  final String name; final String mobile; final String password; final String referral;
  ...
  Future<void> verifyOtp() async {
    ...
    try {
      ...
      if (!doc.exists) {
        ...
        await FirebaseFirestore.instance.collection("users").doc(widget.mobile).set({"name": widget.name, "mobile": widget.mobile, "password": widget.password, "wallet": 0, "upi": "", "isPremium": false, "referralCode": widget.mobile, "referredBy": refBy, "premiumExpiry": Timestamp.now(), "premiumDistributed": false, "createdAt": FieldValue.serverTimestamp()});
      } else {
        await FirebaseFirestore.instance.collection("users").doc(widget.mobile).update({"name": widget.name});
      }
    ...
    } catch (e) { if (kDebugMode) debugPrint("Error $e"); }
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF0A0E1A), appBar: AppBar(title: Text("OTP ${widget.mobile}"), backgroundColor: Colors.amber, foregroundColor: Colors.black),
      body: Center(child: Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Icon(Icons.sms, size: 60, color: Colors.amber), const SizedBox(height: 20),
...
class _HomePageState extends State<HomePage> {
  int wallet = 0; String upi = ""; String myCode = ""; String referredBy = ""; bool isPrem = false; String expiry = ""; String myName = "";
  ...
  void listenUser() {
    FirebaseFirestore.instance.collection("users").doc(widget.mobile).snapshots().listen((d) {
      ...
      if (data["isPremium"] == true) { if (data["premiumExpiry"]!= null) { DateTime exp = (data["premiumExpiry"] as Timestamp).toDate(); if (exp.isAfter(DateTime.now())) { setState(() { isPrem = true; expiry = "${exp.day}/${exp.month}/${exp.year}"; }); } } }
    });
  }
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF0A0E1A), body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
      Row(children: [const Icon(Icons.casino, color: Colors.amber), const SizedBox(width: 8), Text(myName.isEmpty? widget.mobile: myName, style: const TextStyle(color: Colors.white)), const Spacer(), Text("Rs $wallet", style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)), const SizedBox(width: 8), InkWell(onTap: doLogout, child: const Icon(Icons.logout, color: Colors.white54))]),
      ...
      const SizedBox(height: 12), Text("My Code: $myCode", style: const TextStyle(color: Colors.white54, fontSize: 11)), const SizedBox(height: 16),
      ...
      const SizedBox(height: 12),
      Row(children: [Expanded(child: ElevatedButton(onPressed: () { Navigator.push(context, MaterialPageRoute(builder: (_) => WalletScreen(mobile: widget.mobile))); }, child: const Text("WALLET"))), const SizedBox(width: 8), Expanded(child: ElevatedButton(onPressed: () { Navigator.push(context, MaterialPageRoute(builder: (_) => MyTeamScreen(mobile: widget.mobile))); }, child: const Text("MY TEAM")))]),
      const SizedBox(height: 12),
      ...
      const SizedBox(height: 12),
      Row(children: [Expanded(child: ElevatedButton(onPressed: () { Share.share("My Code $myCode"); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.white24), child: const Text("SHARE", style: TextStyle(color: Colors.white)))), const SizedBox(width: 8), Expanded(child: ElevatedButton(onPressed: () { Navigator.push(context, MaterialPageRoute(builder: (_) => WalletScreen(mobile: widget.mobile))); }, child: const Text("WALLET")))]),
      const SizedBox(height: 12), SizedBox(width: double.infinity, child: ElevatedButton(onPressed: isPrem? null : openPremium, style: ElevatedButton.styleFrom(backgroundColor: Colors.purple), child: Text(isPrem? "PREMIUM ACTIVE" : "BUY PREMIUM Rs 500"))),
      const SizedBox(height: 12), TextButton(onPressed: openHelp, child: const Text("Help: +447397293594"))
    ]))));
  }
}
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<void> loadWallet() async { var d = await FirebaseFirestore.instance.collection("users").doc(widget.mobile).get(); if (d.exists) { setState(() { wallet = d.data()!["wallet"]?? 0; upiCtrl.text = d.data()!["upi"]?? ""; loading = false; }); } }
  Future<void> saveUpi() async { await FirebaseFirestore.instance.collection("users").doc(widget.mobile).update({"upi": upiCtrl.text.trim()}); if (!mounted) return; Navigator.pop(context); }
...
class _PremiumPayScreenState extends State<PremiumPayScreen> {
  ...
  Future<void> sendSS() async { final picker = ImagePicker(); final XFile? img = await picker.pickImage(source: ImageSource.gallery); if (img == null) return; setState(() { loading = true; }); await FirebaseFirestore.instance.collection("premium_requests").doc(widget.mobile).set({"mobile": widget.mobile, "amount": 500, "status": "CHECK", "time": Timestamp.now()}); String num = "447397293594"; Uri wa = Uri.parse("https://wa.me/$num?text=Check ${widget.mobile}"); await launchUrl(wa, mode: LaunchMode.externalApplication); setState(() { loading = false; }); }
...
class _MyTeamScreenState extends State<MyTeamScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchTeam() async {
    ...
    var e = await FirebaseFirestore.instance.collection("earnings").where("to", isEqualTo: widget.mobile).get(); earn = e.docs;
    setState(() { loading = false; });
  }
  Widget buildList(List<DocumentSnapshot> list, String emptyMsg) { if (list.isEmpty) { return Center(child: Text(emptyMsg, style: TextStyle(color: Colors.white54))); } return ListView.builder(itemCount: list.length, itemBuilder: (ctx, i) { var data = list[i].data() as Map<String, dynamic>; String name = data["name"]?? "User"; return ListTile(title: Text(name, style: TextStyle(color: Colors.white)), subtitle: Text(data["isPremium"] == true? "Active" : "Free", style: TextStyle(color: Colors.white54))); }); }
  Widget buildEarnList() { if (earn.isEmpty) { return Center(child: Text("No earning", style: TextStyle(color: Colors.white54))); } return ListView.builder(itemCount: earn.length, itemBuilder: (ctx, i) { var d = earn[i].data() as Map; return ListTile(title: Text("${d["type"]} - Rs ${d["amount"]}", style: TextStyle(color: Colors.white)), subtitle: Text("From ${d["from"]}", style: TextStyle(color: Colors.white54))); }); }
  @override Widget build(BuildContext context) { return Scaffold(backgroundColor: const Color(0xFF0A0E1A), appBar: AppBar(backgroundColor: Colors.amber, title: const Text("MY TEAM"), bottom: TabBar(controller: tabCtrl, tabs: const [Tab(text: "L1"), Tab(text: "L2"), Tab(text: "L3"), Tab(text: "EARN")])), body: loading? const Center(child: CircularProgressIndicator()) : TabBarView(controller: tabCtrl, children: [buildList(l1, "Level 1 empty"), buildList(l2, "Level 2 empty"), buildList(l3, "Level 3 empty"), buildEarnList()])); }
}