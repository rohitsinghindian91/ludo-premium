...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<void> saveUpi() async {
    ...
    if (newUpi.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("UPI dalo"))); return; }
    ...
    bool? ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(backgroundColor: Color(0xFF1E1E2E), title: const Text("Password Daalo", style: TextStyle(color: Colors.white, fontSize: 13)), content: TextField(controller: passCtrl, obscureText: true, style: const TextStyle(color: Colors.white), decoration: InputDecoration(hintText: "Login Password", filled: true, fillColor: Color(0xFF151A2B), border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)))), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("Cancel")), ElevatedButton(onPressed: () => Navigator.pop(context, true), style: ElevatedButton.styleFrom(backgroundColor: Colors.amber), child: const Text("Verify"))])));
    if (ok!= true) return;
    if (passCtrl.text.trim()!= myPassword) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Password galat"))); return; }
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 12),
            loading? const CircularProgressIndicator() : SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: sendSS, style: ElevatedButton.styleFrom(backgroundColor: Colors.amber), child: const Text("SEND SCREENSHOT"))),
          ],
        ),
      ),
    );
...
class MyTeamScreen extends StatefulWidget {
  ...
  @override State<MyTeamScreen> createState() => _MyTeamScreenState();
}
...
class _MyTeamScreenState extends State<MyTeamScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchTeam() async {
    ...
    for (var d in q1.docs) { var q = await FirebaseFirestore.instance.collection("users").where("referredBy", isEqualTo: d.id).get(); temp2.addAll(q.docs); }
    l2 = temp2;
    List<DocumentSnapshot> temp3 = [];
    for (var d in temp2) { var q = await FirebaseFirestore.instance.collection("users").where("referredBy", isEqualTo: d.id).get(); temp3.addAll(q.docs); }
    l3 = temp3;
