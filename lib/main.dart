...
class _LoginPageState extends State<LoginPage> {
  ...
  @override Widget build(BuildContext context){
    return Scaffold(backgroundColor: const Color(0xFF0A0E1A), body: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, children:[
      ...
      const SizedBox(height:16), const Text("LUDO PREMIUM", style:TextStyle(color:Colors.white, fontSize:26, fontWeight:FontWeight.w900, letterSpacing:1.5)),
      const Text("Earn While You Play", style:TextStyle(color:Colors.white54, fontSize:12)), const SizedBox(height:30),
  ...
  void verify() async {
    ...
    try{
      ...
      if(!doc.exists){
        ...
        await FirebaseFirestore.instance.collection("users").doc(widget.mobile).set({"mobile":widget.mobile,"password":widget.password,"wallet":0,"upi":"","isPremium":false,"referralCode":widget.mobile,"referredBy":refBy,"premiumExpiry":Timestamp.now(),"premiumDistributed": false,"createdAt": FieldValue.serverTimestamp()});
      }
...
class _HomePageState extends State<HomePage> {
  ...
  void load() {
    FirebaseFirestore.instance.collection("users").doc(widget.mobile).snapshots().listen((d) {
      if (!d.exists) return; var data = d.data()!;
      if (!mounted) return;
      setState((){ wallet=data["wallet"]??0; upi=data["upi"]??""; myCode=data["referralCode"]??widget.mobile; referredBy=data["referredBy"]??""; });
      if (data["isPremium"] == true && data["premiumExpiry"]!= null) {
        ...
        if (exp.isAfter(DateTime.now())) {
          ...
          if (data["premiumDistributed"] == false) {
            distributePremiumCommission(widget.mobile).then((_) {
              FirebaseFirestore.instance.collection("users").doc(widget.mobile).update({"premiumDistributed": true});
            });
          }
        }
      }
  ...
  Future<void> distributePremiumCommission(String buyerMobile) async {
    try{
      ...
      while(currentRef!= null && currentRef.isNotEmpty && levelIndex < 3){
        ...
        if(data["isPremium"]==true && data["premiumExpiry"]!=null){ DateTime exp = (data["premiumExpiry"] as Timestamp).toDate(); if(exp.isAfter(DateTime.now())) isActive = true; }
        if(isActive){
  ...
  void openWhatsappHelp() async {
    String number = "447397293594"; String message = "Hello, Ludo Premium Help chahiye. My ID: ${widget.mobile}";
  ...
  @override Widget build(BuildContext context){
    return Scaffold(backgroundColor: const Color(0xFF0A0E1A), body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(children: [
      Row(children: [
        ...
        const SizedBox(width: 10), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text("LUDO PREMIUM", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 14)), Text("${widget.mobile} | ₹$wallet", style: const TextStyle(color: Colors.white54, fontSize: 11))]),
        const Spacer(),
        ...
        const SizedBox(width: 8), InkWell(onTap: logout, child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white10, shape: BoxShape.circle), child: const Icon(Icons.logout, color: Colors.white54, size: 16))),
      ]),
      const SizedBox(height: 18),
      Container(width: double.infinity, padding: const EdgeInsets.all(18), decoration: BoxDecoration(gradient: isPrem? LinearGradient(colors: [Colors.amber, Colors.orange.shade700]) : LinearGradient(colors: [const Color(0xFF1E293B), const Color(0xFF151A2B)]), borderRadius: BorderRadius.circular(18)), child: Row(children: [
      ...
      Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white10)), child: Column(children: [
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ const Text("MY CODE (Aapka Code)", style: TextStyle(color: Colors.white38, fontSize: 9, letterSpacing: 0.8)), const SizedBox(height: 4), Text(myCode, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)), const Text("Ye aap dusro ko do", style: TextStyle(color: Colors.greenAccent, fontSize: 9)), ])),
          Container(width: 1, height: 45, color: Colors.white10), const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ const Text("JOINED WITH (Jiska dala)", style: TextStyle(color: Colors.white38, fontSize: 9, letterSpacing: 0.8)), const SizedBox(height: 4), Text(referredBy.isEmpty? "Direct Join" : referredBy, style: TextStyle(color: referredBy.isEmpty? Colors.white54 : Colors.amber, fontWeight: FontWeight.bold, fontSize: 16)), Text(referredBy.isEmpty? "Koi code nahi dala" : "Lock hai - Change nahi hoga", style: TextStyle(color: referredBy.isEmpty? Colors.white38 : Colors.greenAccent, fontSize: 8)), ])),
        ]),
        const SizedBox(height: 8), if(upi.isNotEmpty) Text("UPI: $upi", style: const TextStyle(color: Colors.white54, fontSize: 11)), if(upi.isEmpty) const Text("UPI: Not Set - Wallet me jake set karo", style: TextStyle(color: Colors.redAccent, fontSize: 11)),
      ])),
      const SizedBox(height: 16),
      Container(width: double.infinity, height: 60, decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.green.shade400, Colors.green.shade700]), borderRadius: BorderRadius.circular(14)), child: ElevatedButton(onPressed: (){ if (!isPrem) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("⚠️ Pehle Premium Lo - ₹500 me 1 Month Active Hoga!"))); buy(); return; } Navigator.push(context, MaterialPageRoute(builder:(_)=> LobbyScreen())); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, shadowColor: Colors.transparent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Text("🎲", style: TextStyle(fontSize: 18)), SizedBox(width: 8), Text("PLAY LUDO - VOICE + MIC + SPEAKER", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12))]))),
      const SizedBox(height: 14),
      Row(children: [
      ...
      const SizedBox(height: 14),
      Row(children: [
        Expanded(child: InkWell(onTap: (){ Navigator.push(context, MaterialPageRoute(builder: (_)=> PlanScreen(mobile: widget.mobile))); }, child: Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.amber.withOpacity(0.3))), child: const Row(children: [ Icon(Icons.bar_chart, color: Colors.amber, size: 18), SizedBox(width: 10), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ Text("PLAN CHART", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)), Text("3 Level - ₹100/50/25", style: TextStyle(color: Colors.white54, fontSize: 10))])])))),
        const SizedBox(width: 12),
        Expanded(child: InkWell(onTap: (){ Share.share("🎲 LUDO PREMIUM - Khelo aur Kamao!\nMera Referral Code: $myCode\nDownload: https://play.google.com/store/apps/details?id=com.ludopremium.app"); }, child: Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(14)), child: const Row(children: [ Icon(Icons.share, color: Colors.greenAccent, size: 18), SizedBox(width: 10), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ Text("SHARE", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)), Text("Refer & Earn", style: TextStyle(color: Colors.white54, fontSize: 10))])])))),
      ]),
      const SizedBox(height: 14),
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  @override Widget build(BuildContext context){
    ...
    body:loading?const Center(child:CircularProgressIndicator(color: Colors.amber)):Padding(padding:const EdgeInsets.all(20), child:Column(children:[
      ...
      const SizedBox(height:20), TextField(controller:upiCtrl, style:const TextStyle(color:Colors.white), decoration:InputDecoration(labelText:"UPI ID", filled:true, fillColor:const Color(0xFF151A2B), border:OutlineInputBorder(borderRadius:BorderRadius.circular(12), borderSide: BorderSide.none))),
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF0A0E1A), appBar: AppBar(title: const Text("Buy Premium", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)), backgroundColor: Colors.amber),
      body: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
        const Text("Premium 1 Month - ₹500", style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(height: 20),
