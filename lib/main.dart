...
class _HomePageState extends State<HomePage> {
  ...
  void openHelp() async {
    Uri url = Uri.parse("https://wa.me/447397293594?text=Help ${widget.mobile}");
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: isPrem? [Colors.amber, Colors.orange] : [const Color(0xFF1E293B), const Color(0xFF151A2B)]),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                    color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.white12)),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("My Refer Code (Locked): $myCode",
                        style: const TextStyle(color: Colors.amber, fontSize: 11, fontWeight: FontWeight.bold)),
                    Text("Name Locked: $myName", style: const TextStyle(color: Colors.white54, fontSize: 10)),
                    Text(referredBy.isEmpty? "Referred By: Direct" : "Referred By: ${referredByName.isEmpty? referredBy : referredByName}",
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<void> saveUpi() async {
    ...
    if (existingUpi.isEmpty) {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("UPI Set Ho Gaya")));
    ...
    bool? ok = await showDialog<bool>(
      ...
      builder: (ctx) {
        return AlertDialog(
          ...
          title: const Text("Password Daalo", style: TextStyle(color: Colors.white, fontSize: 13)),
          content: TextField(
            controller: passCtrl,
            obscureText: true,
            style: const TextStyle(color: Colors.white),
    ...
    if (passCtrl.text.trim()!= myPassword) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Password galat")));
      return;
    }
    await FirebaseFirestore.instance.collection("users").doc(widget.mobile).update({"upi": newUpi});
    if (!mounted) return;
...
class _PremiumPayScreenState extends State<PremiumPayScreen> {
  ...
  Future<void> sendSS() async {
    ...
    Uri wa = Uri.parse("https://wa.me/447397293594?text=Check ${widget.mobile}");
               ...
               ? const CircularProgressIndicator()
                : SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: sendSS,
                      style: ElevatedButton.styleFrom(backgroundColor: Colors.amber),
                      child: const Text("SEND SCREENSHOT"),
                    ),
                  ),
          ],
        ),
...
class _MyTeamScreenState extends State<MyTeamScreen> with SingleTickerProviderStateMixin {
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.white12)),
          child: ListTile(
            leading: CircleAvatar(
                backgroundColor: Colors.amber,
                child: Text(name.isNotEmpty? name[0].toUpperCase() : "U", style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
            title: Text(name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("Code: $refCode | Last4: ****$last4",
                    style: const TextStyle(color: Colors.amber, fontSize: 10, fontWeight: FontWeight.bold)),
                Text("Status: $prem", style: const TextStyle(color: Colors.white54, fontSize: 9)),
              ],
            ),
          ),
        );
  ...
  Widget buildEarnList() {
    if (earn.isEmpty) {
      return const Center(child: Text("No earning", style: TextStyle(color: Colors.white54)));
    }
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Container(
          ...
          child: ListTile(
            title: Text("${d["type"]} - Rs ${d["amount"]}",
                style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 12)),
            subtitle: Text("From: ****$last4 | Code: $fromMob", style: const TextStyle(color: Colors.white54, fontSize: 10)),
          ),
        );
      },
    );
