...
class _SplashState extends State<Splash> {
  ...
  Widget build(BuildContext context) => const Scaffold(
      ...
      body: Center(
        ...
        SizedBox(height: 10),
        Text("LUDO PREMIUM", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, letterSpacing: 2))
      ])));
}
...
class _LoginPageState extends State<LoginPage> {
  ...
  void goOtp() async {
    ...
    if (passCtrl.text.trim().length < 4) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Password 4 digit se zyada dalo")));
      return;
    }
      ...
      body: Center(
          child: SingleChildScrollView(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Container(
                    padding: const EdgeInsets.all(20),
                    decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.amber, Colors.orange.shade700]), shape: BoxShape.circle),
                    child: const Icon(Icons.casino_rounded, size: 50, color: Colors.black)),
                ...
                TextField(
                    ...
                    style: const TextStyle(color: Colors.white),
                    decoration: InputDecoration(
                        labelText: "Referral Code (Optional) - 1st Time Lock",
                ...
                SizedBox(
                    ...
                    child: ElevatedButton(
                        ...
                        child: const Text("LOGIN / GET OTP 1234", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 14)))),
              ]))),
    );
  }
}
...
class OtpPage extends StatefulWidget {
  ...
  const OtpPage({super.key, required this.mobile, required this.password, required this.referral});
  @override
...
class _OtpPageState extends State<OtpPage> {
  ...
  void verify() async {
    ...
    try {
      ...
      if (mounted &&!kReleaseMode) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString())));
    }
    if (mounted) setState(() {
      load = false;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        backgroundColor: const Color(0xFF0A0E1A),
        appBar: AppBar(title: Text("OTP ${widget.mobile}"), backgroundColor: Colors.amber, foregroundColor: Colors.black),
        body: Center(
            child: Padding(
                padding: const EdgeInsets.all(20),
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const Text("Direct OTP = 1234", style: TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)),
                     ...
                     ? const CircularProgressIndicator(color: Colors.amber)
                      : SizedBox(
                          ...
                          child: ElevatedButton(
                              onPressed: verify,
                              style: ElevatedButton.styleFrom(backgroundColor: Colors.amber, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
                              child: const Text("VERIFY 1234", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900)))),
                ]))));
  }
}
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> distributePremiumCommission(String buyerMobile) async {
    try {
      var buyerDoc = await FirebaseFirestore.instance.collection("users").doc(buyerMobile).get();
      if (!buyerDoc.exists) return;
      String? currentRef = buyerDoc.data()?["referredBy"];
      List<int> commissions = [100, 50, 25];
      int levelIndex = 0;
      while (currentRef!= null && currentRef.isNotEmpty && levelIndex < 3) {
        var refDoc = await FirebaseFirestore.instance.collection("users").doc(currentRef).get();
        if (!refDoc.exists) break;
        var data = refDoc.data()!;
        bool isActive = false;
        if (data["isPremium"] == true && data["premiumExpiry"]!= null) {
          DateTime exp = (data["premiumExpiry"] as Timestamp).toDate();
          if (exp.isAfter(DateTime.now())) isActive = true;
        }
        if (isActive) {
          ...
          await FirebaseFirestore.instance.collection("earnings").add({
            "to": currentRef,
            "from": buyerMobile,
            "type": "premium_level_${levelIndex + 1}",
            "amount": commissions[levelIndex],
      ...
      if (kDebugMode) debugPrint("Commission error");
    }
  }

  void buy() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => PremiumPayScreen(mobile: widget.mobile, onPaid: () {
          load();
        }))).then((_) => load());
  }
            ...
            children: [
              Row(
                children: [
                  Container(
                      ...
                      decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.amber, Colors.orange.shade700]), borderRadius: BorderRadius.circular(12)),
                      child: const Icon(Icons.casino, color: Colors.black, size: 20)),
                  const SizedBox(width: 10),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text("LUDO PREMIUM", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 14)),
                    Text("${widget.mobile} | ₹$wallet", style: const TextStyle(color: Colors.white54, fontSize: 11))
                  ]),
                  const Spacer(),
                  Container(
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
                      decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.amber.withOpacity(0.2))),
                      child: Row(children: [
                        const Icon(Icons.account_balance_wallet, color: Colors.amber, size: 14),
                        const SizedBox(width: 5),
                        Text("₹$wallet", style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold, fontSize: 12))
                      ])),
                  const SizedBox(width: 8),
                  InkWell(
                      onTap: logout,
                      child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white10, shape: BoxShape.circle), child: const Icon(Icons.logout, color: Colors.white54, size: 16))),
                ],
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: isPrem? LinearGradient(colors: [Colors.amber, Colors.orange.shade700]) : LinearGradient(colors: [Color(0xFF1E293B), Color(0xFF151A2B)]),
                ...
                child: Row(
                  children: [
                    Container(
                        ...
                        decoration: BoxDecoration(color: isPrem? Colors.black.withOpacity(0.15) : Colors.white10, borderRadius: BorderRadius.circular(12)),
                        child: Icon(isPrem? Icons.workspace_premium : Icons.lock, color: isPrem? Colors.black : Colors.white54, size: 26)),
              ...
              Container(
                ...
                child: ElevatedButton(
                    ...
                    child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      SizedBox(width: 8),
                      Text("PLAY LUDO - VOICE + MIC + SPEAKER", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12))
                    ])),
              ),
              const SizedBox(height: 14),
              Row(children: [
                Expanded(
                    child: InkWell(
                        onTap: () {
                          Navigator.push(context, MaterialPageRoute(builder: (_) => WalletScreen(mobile: widget.mobile))).then((_) => load());
                        },
                        child: Container(
                            padding: const EdgeInsets.all(14),
                            decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(14)),
                            child: const Row(children: [
                              Icon(Icons.account_balance_wallet, color: Colors.blueAccent, size: 18),
                              SizedBox(width: 10),
                              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Text("WALLET", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                                Text("Manage", style: TextStyle(color: Colors.white54, fontSize: 10))
                              ])
                            ])))),
                const SizedBox(width: 12),
                Expanded(
                    child: InkWell(
                        onTap: () {
                          Navigator.push(context, MaterialPageRoute(builder: (_) => MyTeamScreen(mobile: widget.mobile)));
                        },
                        child: Container(
                            padding: const EdgeInsets.all(14),
                            decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(14)),
                            child: const Row(children: [
                              Icon(Icons.groups, color: Colors.purpleAccent, size: 18),
                              SizedBox(width: 10),
                              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Text("MY TEAM", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                                Text("View Team", style: TextStyle(color: Colors.white54, fontSize: 10))
                              ])
                            ])))),
              ]),
              const SizedBox(height: 14),
              InkWell(
                  ...
                  child: Container(
                      ...
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(
                          gradient: isPrem? null : LinearGradient(colors: [Color(0xFF6A11CB), Color(0xFF2575FC)]),
                          color: isPrem? Colors.white10 : null,
                          borderRadius: BorderRadius.circular(14)),
                      child: Row(children: [
                        Container(
                            padding: const EdgeInsets.all(10),
                            decoration: BoxDecoration(color: Colors.white.withOpacity(0.15), borderRadius: BorderRadius.circular(10)),
                            child: Icon(isPrem? Icons.check : Icons.workspace_premium, color: Colors.white, size: 18)),
                        const SizedBox(width: 12),
                        Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(isPrem? "PREMIUM ACTIVE ✅" : "BUY PREMIUM ₹500", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                          Text(isPrem? "Enjoy unlimited play" : "1 Month • Unlock All Features", style: const TextStyle(color: Colors.white70, fontSize: 11))
                        ]))
                      ]))),
              const SizedBox(height: 18),
              InkWell(
                  ...
                  child: Container(
                      ...
                      child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        ...
                        SizedBox(width: 10),
                        Text("Help: +447397293594", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14))
                      ]))),
            ],
          ),
        ),
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF0A0E1A),
      appBar: AppBar(title: const Text("Wallet", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)), backgroundColor: Colors.amber, iconTheme: const IconThemeData(color: Colors.black)),
      body: loading
          ...
          : Padding(
              ...
              child: Column(children: [
                Container(
                    ...
                    child: Column(children: [
                      const Text("Total Wallet", style: TextStyle(color: Colors.black87, fontSize: 12)),
                      Text("₹$wallet", style: const TextStyle(color: Colors.black, fontSize: 36, fontWeight: FontWeight.w900))
                    ])),
                const SizedBox(height: 20),
                ...
                SizedBox(
                    width: double.infinity,
                    height: 52,
                    child: ElevatedButton(onPressed: save, style: ElevatedButton.styleFrom(backgroundColor: Colors.amber), child: const Text("SAVE UPI", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900)))),
              ])),
    );
  }
}
...
class _PremiumPayScreenState extends State<PremiumPayScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF0A0E1A),
      appBar: AppBar(title: const Text("Buy Premium", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)), backgroundColor: Colors.amber),
      body: Padding(
          padding: const EdgeInsets.all(20),
          child: Column(children: [
            ...
            Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(14)),
                child: Column(children: [
                  const Text("Is UPI pe ₹500 bhejo:", style: TextStyle(color: Colors.white70, fontSize: 12)),
                  const SizedBox(height: 8),
               ...
               ? const CircularProgressIndicator(color: Colors.amber)
                : SizedBox(
                    ...
                    child: ElevatedButton.icon(
                        onPressed: pickAndSendDirectWhatsapp,
                        style: ElevatedButton.styleFrom(backgroundColor: Colors.amber),
                        icon: const Icon(Icons.camera_alt, color: Colors.black),
                        label: const Text("STEP 2: SCREENSHOT WHATSAPP", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 12)))),
          ])),
    );
  }
}
...
class _MyTeamScreenState extends State<MyTeamScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchTeam() async {
    ...
    try {
      ...
      level1 = l1.docs;
      List<DocumentSnapshot> l2temp = [];
      for (var doc in l1.docs) {
        var q = await FirebaseFirestore.instance.collection("users").where("referredBy", isEqualTo: doc.id).get();
        l2temp.addAll(q.docs);
      }
      level2 = l2temp;
      List<DocumentSnapshot> l3temp = [];
      for (var doc in l2temp) {
        var q = await FirebaseFirestore.instance.collection("users").where("referredBy", isEqualTo: doc.id).get();
        l3temp.addAll(q.docs);
      }
      level3 = l3temp;
  ...
  Widget userTile(DocumentSnapshot doc) {
    ...
    return Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFF151A2B), borderRadius: BorderRadius.circular(12)),
        child: ListTile(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
            ...
            title: const Text("MY TEAM", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900)),
            bottom: TabBar(controller: tabCtrl, labelColor: Colors.black, tabs: const [
              Tab(text: "L1 (100)"),
              Tab(text: "L2 (50)"),
              Tab(text: "L3 (25)"),
              Tab(text: "EARNINGS"),
        ...
        body: loading
           ? const Center(child: CircularProgressIndicator(color: Colors.amber))
            : TabBarView(controller: tabCtrl, children: [
                ...
                ListView(
                    children: earnings.isEmpty
                       ? [const Center(child: Padding(padding: EdgeInsets.all(20), child: Text("Abhi koi earning nahi", style: TextStyle(color: Colors.white54))))]
                        : earnings.map((e) {
                            var d = e.data() as Map;
                            return ListTile(
                                title: Text("${d["type"]} - ₹${d["amount"]}", style: const TextStyle(color: Colors.white)),
                                subtitle: Text("From: ${d["from"]}", style: const TextStyle(color: Colors.white54)));
                          }).toList()),
              ]));
  }
}