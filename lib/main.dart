...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> register() async {
    ...
    if (!doc.exists) {
      await userDoc.set({
        'phone': phone,
        'ownReferralCode': phone.substring(phone.length - 6),
    ...
    Navigator.pushReplacement(
        context, MaterialPageRoute(builder: (_) => HomeScreen(phone: phone)));
  }

  Future<void> distributeCommission(String refCode) async {
    var level1Query = await FirebaseFirestore.instance
       .collection('users')
       .where('ownReferralCode', isEqualTo: refCode)
       .get();
    ...
    if (l1UsedCode.isNotEmpty) {
      var level2Query = await FirebaseFirestore.instance
         .collection('users')
         .where('ownReferralCode', isEqualTo: l1UsedCode)
         .get();
      if (level2Query.docs.isNotEmpty) {
        ...
        if (l2UsedCode.isNotEmpty) {
          var level3Query = await FirebaseFirestore.instance
             .collection('users')
             .where('ownReferralCode', isEqualTo: l2UsedCode)
             .get();
          if (level3Query.docs.isNotEmpty) {
            await level3Query.docs.first.reference
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        // YAHI LINE FIX KI HAI -??? HATA DIYA
              ...
              children: [
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("Your Referral Code: $ownCode",
                          style: const TextStyle(
                              color: Colors.amber, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 5),
                      Row(
                        children: [
                          const Text("Joined with Code: ",
                      ...
                      const Text("(Ye change nahi ho sakta)",
            ...
            itemBuilder: (context, i) {
              ...
              return ListTile(
                title: Text(d['phone']?? "",
                    style: const TextStyle(color: Colors.white)),
                subtitle: Text(
                    "Used: ${d['usedReferralCode']} | Own: ${d['ownReferralCode']} | Wallet: ${d['wallet']}",
                    style: const TextStyle(color: Colors.white54)),
                trailing: Text(d['isPremium'] == true? "PREMIUM" : "FREE",
