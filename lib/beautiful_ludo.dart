          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text("DOST KE SATH KHELO", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11)),
  ...
  Future<void> startQuickMatch() async {
    ...
    try {
      ...
      if (foundRoomId!= null && foundKey!= null) {
        ...
        setState(() {
          statusText = "Real Dost mil gaya! Join ho rahe hain...";
        });
...
class _LudoGameState extends State<LudoGame> with SingleTickerProviderStateMixin {
    ...
    if (widget.mode == GameMode.online) {
      ...
      try {
        await agoraEngine!.joinChannel(
          ...
          uid: myUid,
          options: ChannelMediaOptions(clientRoleType: ClientRoleType.clientRoleBroadcaster, channelProfile: ChannelProfileType.channelProfileCommunication),
        );
      ...
      if (myMobile!= null) {
        await playersRef.set({"mobile": myMobile, "name": myName, "player": widget.myPlayer, "joinedAt": ServerValue.timestamp});
      } else {
        await playersRef.set({"mobile": "guest_${Random().nextInt(999999)}", "name": myName, "player": widget.myPlayer, "joinedAt": ServerValue.timestamp});
      }
      ...
      chatRef!.onChildAdded.listen((event) {
        if (event.snapshot.value!= null && mounted) {
          ...
          String msg = data['msg']?? "";
          setState(() {
            chatMessages.add({"player": player, "msg": msg, "time": data['time']});
          });
        }
      });
  ...
  Future<void> addFriendFromGame() async {
    if (widget.mode == GameMode.bot) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("$selectedBotName BOT hai")));
      return;
    }
  ...
  Future<void> sendFriendRequest(String toMobile, String toName) async {
    ...
    if (!doc.exists) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("$toName nahi mila")));
      return;
    }
    ...
    if (q.docs.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("$toName ko pehle se bhej rakhi hai")));
      return;
    }
    var fr = await FirebaseFirestore.instance.collection("users").doc(myMobile!).collection("friends").doc(toMobile).get();
    if (fr.exists) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("$toName pehle se friend hai")));
      return;
    }
  ...
  bool get isMyTurn {
    if (widget.mode == GameMode.offline) return true;
    if (widget.mode == GameMode.bot) return turn == 0;
    return turn == widget.myPlayer;
  }

  bool isValidMove(int p, int idx, int d) {
    int cur = pos[p][idx];
    if (cur == 45) return false;
    if (cur == -1) return d == 6;
    if (cur >= 40) return cur + d <= 45;
    int dist = (homeEntry[p] - cur + 40) % 40;
  ...
  void sendMessage() async {
    ...
    if (widget.mode == GameMode.online && chatRef!= null) {
      ...
      setState(() {
        chatMessages.add({"player": name, "msg": text, "time": DateTime.now().millisecondsSinceEpoch});
      });
    }
  }
  ...
  void syncRoom() {
    if (widget.mode == GameMode.online && roomRef!= null) {
      roomRef!.update({"pos": pos, "turn": turn, "diceGreen": diceGreen, "diceRed": diceRed, "canMove": canMove, "gameOver": gameOver});
    }
  }
    ...
    Future.delayed(Duration(milliseconds: 800), () {
      ...
      if (d == 6) {
        ...
        if (consecutiveSixes == 3) {
          ...
          syncRoom();
          if (widget.mode == GameMode.bot && turn == 1) botTurn();
          return;
        }
      } else {
        consecutiveSixes = 0;
      ...
      syncRoom();
      bool any = false;
      for (int i = 0; i < 4; i++) if (isValidMove(turn, i, d)) {
        any = true;
        break;
      }
      if (!any) {
  ...
  void botTurn() {
    ...
    if (widget.mode!= GameMode.bot) return;
    if (turn!= 1) return;
    rollBot();
  }
  ...
  void rollBot() {
    ...
    Future.delayed(Duration(milliseconds: 800), () {
      ...
      bool any = false;
      for (int i = 0; i < 4; i++) if (isValidMove(1, i, d)) {
        any = true;
        break;
      }
      if (!any) {
  ...
  void botMove() {
    ...
    int bestIdx = -1;
    for (int i = 0; i < 4; i++) {
      if (!isValidMove(1, i, diceRed)) continue;
      int cur = pos[1][i];
      if (cur == -1) {
        bestIdx = i;
        break;
      ...
      if (cur < 40) {
        int next = (cur + diceRed) % 40;
        if (!safe.contains(next)) {
          for (int k = 0; k < 4; k++) if (pos[0][k] == next) {
            bestIdx = i;
            break;
          }
        }
  ...
  void moveGoti(int idx) {
    ...
    setState(() {
      ...
      if (cur == -1) pos[turn][idx] = startPos[turn];
        ...
        int dist = (homeEntry[turn] - cur + 40) % 40;
        if (dice == dist + 1) pos[turn][idx] = 40;
        else {
          int next = (cur + dice) % 40;
          if (!safe.contains(next)) {
            for (int k = 0; k < 4; k++) if (pos[opp][k] == next) {
              pos[opp][k] = -1;
              gotCut = true;
            }
          }
    ...
    if (isWin) {
      Future.delayed(Duration(milliseconds: 200), () {
        ...
        showDialog(
          ...
          builder: (_) => AlertDialog(
            backgroundColor: Color(0xFF1E1E2E),
            title: Text("${turn == 0? myName : opponentName} JEET GAYA!", style: TextStyle(color: Colors.white)),
            actions: [
              TextButton(
                onPressed: () {
                  Navigator.pop(context);
                  setState(() {
                    ...
                    consecutiveSixes = 0;
                    selectedBotName = botNames[_rng.nextInt(botNames.length)];
                    if (widget.mode == GameMode.bot) opponentName = selectedBotName;
                  });
                  syncRoom();
                  if (widget.mode == GameMode.bot && turn == 1) botTurn();
                },
      ...
      if (widget.mode == GameMode.bot && turn == 1) Future.delayed(Duration(milliseconds: 600), () => botTurn());
    }
  }

  Offset getHomePathPos(int p, int step, double s) {
    double r = s * 0.25, cx = s / 2, cy = s / 2;
    int entry = homeEntry[p];
    double ang = (entry / 40) * 2 * pi - pi / 2;
    double ex = cx + r * cos(ang), ey = cy + r * sin(ang);
    double t = (step + 1) / 6.0;
  ...
  Widget buildDiceFace(int v) {
    Widget d = dot(), e = emptyDot();
    List<Widget> r1 = [e, e, e], r2 = [e, e, e], r3 = [e, e, e];
    if (v == 1) r2 = [e, d, e];
    else if (v == 2) { r1 = [d, e, e]; r3 = [e, e, d]; }
    else if (v == 3) { r1 = [d, e, e]; r2 = [e, d, e]; r3 = [e, e, d]; }
    else if (v == 4) { r1 = [d, e, d]; r3 = [d, e, d]; }
    else if (v == 5) { r1 = [d, e, d]; r2 = [e, d, e]; r3 = [d, e, d]; }
    else if (v == 6) { r1 = [d, e, d]; r2 = [d, e, d]; r3 = [d, e, d]; }
    return Container(
      width: 68,
      height: 68,
  ...
  Widget diceNearHome(int p, int val) {
    bool isTurn = turn == p &&!canMove &&!gameOver;
    bool canTap = isTurn &&!isRolling && isMyTurn;
    if (widget.mode == GameMode.bot && p == 1) canTap = false;
    Color col = p == 0? Colors.green : Colors.red;
    bool rolling = isRolling && turn == p;
    return GestureDetector(
      onTap: canTap? roll : null,
      child: AnimatedBuilder(
        animation: _diceController,
        builder: (c, child) {
          double a = rolling? _diceController.value * 4 * pi : 0;
        ...
        child: AnimatedContainer(
          ...
          child: Center(child: rolling? buildDiceFace(_rng.nextInt(6) + 1) : buildDiceFace(val)),
        ),
      ),
    );
  }
  ...
  Widget goti(int p, int t, double s) {
    int v = pos[p][t];
    double boxSize = s * 0.14, pad = s * 0.02;
    Offset o;
    if (v == -1) {
      if (p == 0) {
        double bx = 10 + pad, by = 10 + pad;
        o = Offset(bx + (t % 2) * (boxSize / 2.2), by + (t ~/ 2) * (boxSize / 2.2));
      } else {
        double bx = s - 10 - boxSize + pad, by = s - 10 - boxSize + pad;
        o = Offset(bx + (t % 2) * (boxSize / 2.2), by + (t ~/ 2) * (boxSize / 2.2));
      }
    } else if (v == 45) {
      o = Offset(s / 2 + (t % 2 == 0? -8 : 8), s / 2 + (t < 2? -8 : 8));
    } else if (v >= 40) {
      o = getHomePathPos(p, v - 40, s);
    } else {
      double r = s * 0.25, ang = (v / 40) * 2 * pi - pi / 2;
      o = Offset(s / 2 + r * cos(ang), s / 2 + r * sin(ang));
    }
    bool act = p == turn && canMove && isValidMove(p, t, dice) &&!gameOver && isMyTurn;
    if (widget.mode == GameMode.offline) act = p == turn && canMove && isValidMove(p, t, dice) &&!gameOver;
    return Positioned(
      left: o.dx - 11,
      top: o.dy - 11,
      child: GestureDetector(
        onTap: act? () => moveGoti(t) : null,
        child: Container(
          width: act? 28 : 20,
          height: act? 28 : 20,
          decoration: BoxDecoration(color: p == 0? Colors.green : Colors.red, shape: BoxShape.circle, border: Border.all(color: act? Colors.yellow : Colors.white, width: act? 2.5 : 1.5)),
        ),
      ),
    );
  }
  ...
  @override
  Widget build(BuildContext context) {
    double s = (MediaQuery.of(context).size.width < 400? MediaQuery.of(context).size.width : 400) - 20;
    double box = s * 0.14;
    return Scaffold(
      backgroundColor: Color(0xFF0A0E1A),
      appBar: AppBar(
          ...
          if (widget.mode == GameMode.online)
            IconButton(
              ...
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Room code copied: ${widget.roomId}")));
              },
            ),
      ...
      body: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(color: Color(0xFF151A2B), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.amber.withOpacity(0.3))),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text("Turn: ${turn == 0? myName : opponentName}", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.white)),
            Text(isMyTurn? "YOUR TURN - TAP DICE" : "WAIT - $opponentName", style: TextStyle(fontSize: 11, color: isMyTurn? Colors.greenAccent : Colors.white54, fontWeight: FontWeight.bold)),
          ]),
        ),
        Expanded(
          child: Stack(children: [
            Center(
              child: Container(
                ...
                child: Stack(clipBehavior: Clip.none, children: [
                  ...
                  for (int i = 0; i < 40; i++)
                    Positioned(
                      left: s / 2 + s * 0.25 * cos((i / 40) * 2 * pi - pi / 2) - 7,
                      top: s / 2 + s * 0.25 * sin((i / 40) * 2 * pi - pi / 2) - 7,
                      child: Container(
                        width: 14,
                        height: 14,
                        decoration: BoxDecoration(color: safe.contains(i)? Color(0xFFFFD700) : Colors.white, shape: BoxShape.circle, border: Border.all(color: safe.contains(i)? Colors.orange : Colors.black26, width: safe.contains(i)? 1.5 : 1)),
                      ),
                    ),
                  for (int j = 0; j < 5; j++)
                    Positioned(left: getHomePathPos(0, j, s).dx - 7, top: getHomePathPos(0, j, s).dy - 7, child: Container(width: 14, height: 14, decoration: BoxDecoration(color: Colors.green.shade200, shape: BoxShape.circle, border: Border.all(color: Colors.green.shade400)))),
                  for (int j = 0; j < 5; j++)
                    Positioned(left: getHomePathPos(1, j, s).dx - 7, top: getHomePathPos(1, j, s).dy - 7, child: Container(width: 14, height: 14, decoration: BoxDecoration(color: Colors.red.shade200, shape: BoxShape.circle, border: Border.all(color: Colors.red.shade400)))),
                  goti(0, 0, s),
                  goti(0, 1, s),
                  goti(0, 2, s),
                  goti(0, 3, s),
              ...
              child: Center(
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    GestureDetector(
                      onTap: toggleMic,
                      child: Container(padding: EdgeInsets.all(8), decoration: BoxDecoration(color: isMicOn? Colors.green : Colors.red, shape: BoxShape.circle), child: Icon(isMicOn? Icons.mic : Icons.mic_off, color: Colors.white, size: 18)),
                    ),
                    SizedBox(width: 10),
                    GestureDetector(
                      onTap: toggleSpeaker,
                      child: Container(padding: EdgeInsets.all(8), decoration: BoxDecoration(color: isSpeakerOn? Colors.green : Colors.red, shape: BoxShape.circle), child: Icon(isSpeakerOn? Icons.volume_up : Icons.volume_off, color: Colors.white, size: 18)),
                    ),
                    SizedBox(width: 10),
            ...
            if (showChat)
              Positioned(
                ...
                child: Container(
                  height: 380,
                  decoration: BoxDecoration(color: Color(0xFF151A2B), borderRadius: BorderRadius.only(topLeft: Radius.circular(16), topRight: Radius.circular(16)), border: Border.all(color: Colors.amber.withOpacity(0.5))),
                  child: Column(children: [
                    Container(
                      padding: EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                      decoration: BoxDecoration(color: Colors.amber, borderRadius: BorderRadius.only(topLeft: Radius.circular(16), topRight: Radius.circular(16))),
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Text("CHAT - $myName vs $opponentName", style: TextStyle(color: Colors.black, fontSize: 13, fontWeight: FontWeight.bold)),
                        GestureDetector(
                          onTap: () => setState(() => showChat = false),
                          child: Container(padding: EdgeInsets.all(4), decoration: BoxDecoration(color: Colors.black, shape: BoxShape.circle), child: Icon(Icons.close, color: Colors.white, size: 16)),
                        ),
                      ]),
                    ),
                    Expanded(
                        ...
                        itemBuilder: (c, i) {
                          ...
                          return Align(
                            ...
                            child: Container(
                              ...
                              child: Text("${m['player']}: ${m['msg']}", style: TextStyle(fontSize: 13, color: Colors.white)),
                            ),
                          );
                        },
                      ),
        ...
        Container(
          ...
          decoration: BoxDecoration(color: Color(0xFF151A2B), borderRadius: BorderRadius.circular(20), border: Border.all(color: turn == 0? Colors.green : Colors.red, width: 1.5)),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.casino, color: turn == 0? Colors.green : Colors.red, size: 16),
            SizedBox(width: 6),
            Text("${turn == 0? myName : opponentName} KI BAARI ${isMyTurn? "(TAP DICE)" : ""}", style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
          ]),
        ),
      ]),
