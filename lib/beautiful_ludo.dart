...
// QUICK FIX - bina persistence ke, persistence sirf main.dart me hai
...
class _LobbyScreenState extends State<LobbyScreen> {
  ...
  void _createRoomWithCodeDialog() async {
    ...
    } catch(e){ ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Room nahi bana: $e"))); }
  }
  void joinRoom() async {
    await ensurePrefs();
...
class _QuickMatchScreenState extends State<QuickMatchScreen> {
  ...
  @override void initState() { super.initState(); queueRef.keepSynced(true); startQuickMatch(); }
  @override void dispose() { _timer?.cancel(); super.dispose(); }
  void launchBot() { if(botLaunched) return; botLaunched = true; _timer?.cancel(); searching = false; if(!mounted) return; Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => LudoGame(roomId: "BOT", myPlayer: 0, mode: GameMode.bot))); }
  Future<void> startQuickMatch() async {
    _timer = Timer.periodic(Duration(seconds: 1), (timer) { if(!mounted){ timer.cancel(); return; } if(countdown>0){ setState((){ countdown--; }); } else { launchBot(); } });
    try {
      ...
      if(foundRoomId!=null && foundKey!=null){ if(!mounted || botLaunched) return; setState((){ statusText = "Real Dost mil gaya!"; }); await queueRef.child(foundKey).update({"status": "matched", "guestId": myId}); await Future.delayed(Duration(milliseconds: 500)); if(!mounted || botLaunched) return; _timer?.cancel(); botLaunched = true; Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => LudoGame(roomId: foundRoomId!, myPlayer: 1, mode: GameMode.online))); return; }
    ...
    } catch(e){ debugPrint("Queue error $e"); }
  }
  @override Widget build(BuildContext context){ return Scaffold(backgroundColor: Color(0xFF0A0E1A), body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircularProgressIndicator(color: Colors.orange, strokeWidth: 6), SizedBox(height: 30), Text(statusText, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)), SizedBox(height: 20), Text("$countdown", style: TextStyle(color: Colors.amber, fontSize: 72, fontWeight: FontWeight.w900))] ))); }
}
...
class _LudoGameState extends State<LudoGame> with SingleTickerProviderStateMixin {
  ...
  Future<void> initAgoraAndRoom() async {
    try {
      await ensurePrefs();
      myMobile = ludoPrefs.getString("mobile");
    ...
    if(widget.mode == GameMode.online){
      ...
      agoraEngine!.registerEventHandler(RtcEngineEventHandler(
        onJoinChannelSuccess: (RtcConnection connection, int elapsed) { setState(()=> isAgoraJoined = true); },
      ...
      try {
        await agoraEngine!.joinChannel(token: agoraToken, channelId: widget.roomId, uid: myUid, options: ChannelMediaOptions(clientRoleType: ClientRoleType.clientRoleBroadcaster, channelProfile: ChannelProfileType.channelProfileCommunication, autoSubscribeAudio: true, publishMicrophoneTrack: true));
      ...
      await playersRef!.keepSynced(true);
      ...
      roomRef!.onValue.listen((event){
        if(event.snapshot.value!=null && mounted){
          ...
          setState((){
            if(data['pos']!=null) {
              try { var raw = data['pos'] as List; pos = List<List<int>>.from(raw.map((e)=> List<int>.from((e as List).map((x)=> x as int)))); } catch(_){}
            }
      ...
      // FINAL CONTACT FIX - players node se naam lo
      ...
      if(widget.myPlayer == 0){
        Future.delayed(Duration(milliseconds: 800), (){ roomRef!.get().then((snap){ if(!snap.exists){ syncRoom(); } }); });
      }
  ...
  Future<void> addFriendFromGame() async {
    ...
    if(opponentMobile.isEmpty){ ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Opponent abhi connect nahi hua - naam: $opponentName"))); return; }
  ...
  void sendMessage() async {
    ...
    } else setState(() { chatMessages.add({"player": myName, "msg": text}); });
  }
  ...
  void rollBot() { setState(() => isRolling = true); _diceController.forward(from: 0); Future.delayed(Duration(milliseconds: 800), () { if (!mounted) return; int d = _rng.nextInt(6) + 1; if (d == 6) { consecutiveSixes++; if (consecutiveSixes == 3) { setState(() { diceRed = d; turn = 0; canMove = false; consecutiveSixes = 0; isRolling = false; }); return; } } else consecutiveSixes = 0; setState(() { diceRed = d; canMove = true; isRolling = false; }); bool any = false; for (int i = 0; i < 4; i++) if (isValidMove(1, i, d)) { any = true; break; } if (!any) { Future.delayed(Duration(milliseconds: 500), () { if (!mounted) return; setState(() { turn = 0; canMove = false; }); }); } else Future.delayed(Duration(milliseconds: 500), () => botMove()); }); }
  void botMove() { if (!canMove || gameOver) return; int bestIdx = -1; for (int i = 0; i < 4; i++) { if (!isValidMove(1, i, diceRed)) continue; int cur = pos[1][i]; if (cur == -1) { bestIdx = i; break; } if (cur < 40) { int next = (cur + diceRed) % 40; if (!safe.contains(next)) { for (int k = 0; k < 4; k++) if (pos[0][k] == next) { bestIdx = i; break; } } } if (bestIdx!= -1) break; } if (bestIdx == -1) { int maxPos = -2; for (int i = 0; i < 4; i++) { if (!isValidMove(1, i, diceRed)) continue; if (pos[1][i] > maxPos) { maxPos = pos[1][i]; bestIdx = i; } } } if (bestIdx!= -1) moveGoti(bestIdx); }
  ...
  @override Widget build(BuildContext context) {
    ...
    String turnName;
    if(widget.mode == GameMode.online){ turnName = (turn == widget.myPlayer)? myName : opponentName; } else { turnName = (turn == 0)? myName : opponentName; }
