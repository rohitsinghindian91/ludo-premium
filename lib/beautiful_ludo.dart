...
FirebaseDatabase getRtdb() {
  if (_rtdbInstance == null) {
    _rtdbInstance = FirebaseDatabase.instanceFor(app: Firebase.app(), databaseURL: rtdbUrl);
    try { _rtdbInstance!.setPersistenceEnabled(true); _rtdbInstance!.setPersistenceCacheSizeBytes(10000000); } catch (_) {}
...
class _LobbyScreenState extends State<LobbyScreen> {
  ...
  void _createRoomWithCodeDialog() async {
    ...
    try {
      ...
      if (!mounted) return;
      showDialog(context: context, builder: (_) => AlertDialog(title: Text("Room $c"), content: Text("Code: $c"), actions: [ElevatedButton(onPressed: (){ Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (_) => LudoGame(roomId: c, myPlayer: 0, mode: GameMode.online))); }, child: Text("START"))]));
  ...
  @override Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Color(0xFF0A0E1A), body: Center(child: Padding(padding: EdgeInsets.all(20), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.casino, size: 60, color: Colors.amber), Text("LUDO PREMIUM", style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: Colors.amber)), SizedBox(height: 20),
      SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: _createRoomWithCodeDialog, child: Text("CREATE ROOM"))),
      TextField(controller: codeCtrl, maxLength: 4), SizedBox(width: double.infinity, height: 50, child: ElevatedButton(onPressed: joinRoom, child: Text("JOIN ROOM"))),
    ]))));
  }
}
...
class _QuickMatchScreenState extends State<QuickMatchScreen> {
  int countdown = 10; String statusText = "Searching..."; bool botLaunched = false;
  ...
  Future<void> startQuickMatch() async {
    ...
    var snap = await queueRef.get(); String? foundRoomId; String? foundKey;
    if(snap.exists){ for(var child in snap.children){ var data = Map<String,dynamic>.from(child.value as Map); if(data['status']=='waiting' && data['hostId']!=myId){ foundRoomId = data['roomId']; foundKey = child.key; break; } } }
    if(foundRoomId!=null){ await queueRef.child(foundKey!).update({"status": "matched"}); _timer?.cancel(); Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => LudoGame(roomId: foundRoomId!, myPlayer: 1, mode: GameMode.online))); return; }
    myRoomId = (Random().nextInt(9000)+1000).toString(); myQueueKey = queueRef.push().key!; await queueRef.child(myQueueKey!).set({"roomId": myRoomId, "hostId": myId, "status": "waiting", "createdAt": ServerValue.timestamp});
  }
  @override Widget build(BuildContext context){ return Scaffold(body: Center(child: Text("$statusText $countdown"))); }
}
...
class _LudoGameState extends State<LudoGame> with SingleTickerProviderStateMixin {
  ...
  bool isMicOn = true, isSpeakerOn = true; bool isAgoraJoined = false;
  ...
  Future<void> initAgoraAndRoom() async {
    ...
    if(widget.mode == GameMode.online){
      getRtdb().goOnline();
      try { agoraEngine = createAgoraRtcEngine(); await agoraEngine!.initialize(RtcEngineContext(appId: agoraAppId)); await agoraEngine!.enableAudio(); await agoraEngine!.joinChannel(token: agoraToken, channelId: widget.roomId, uid: widget.myPlayer==0?1:2, options: ChannelMediaOptions(clientRoleType: ClientRoleType.clientRoleBroadcaster, channelProfile: ChannelProfileType.channelProfileCommunication, autoSubscribeAudio: true, publishMicrophoneTrack: true)); } catch(_){}
      ...
      // *** FINAL FIX - pura players node suno, yehi se naam aayega ***
  ...
  Future<void> addFriendFromGame() async {
    if(opponentMobile.isEmpty){ ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Wait karo, naam: $opponentName"))); return; }
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("$opponentName : $opponentMobile")));
  }
  ...
  void sendMessage() async { if (chatCtrl.text.trim().isEmpty) return; String text = chatCtrl.text.trim(); chatCtrl.clear(); if(widget.mode == GameMode.online && chatRef!= null){ getRtdb().goOnline(); await chatRef!.push().set({"player": myName, "msg": text, "time": ServerValue.timestamp}); } }
  void toggleMic() async { setState(() => isMicOn =!isMicOn); }
  void toggleSpeaker() async { setState(() => isSpeakerOn =!isSpeakerOn); }
  ...
  void moveGoti(int idx) {
    if (!canMove || gameOver) return; if (!isValidMove(turn, idx, dice)) return;
    setState(() {
      int cur = pos[turn][idx]; int opp = 1 - turn;
      if (cur == -1) pos[turn][idx] = startPos[turn];
      else if (cur < 40) { int dist = (homeEntry[turn] - cur + 40) % 40; if (dice == dist + 1) pos[turn][idx] = 40; else { int next = (cur + dice) % 40; if (!safe.contains(next)) { for (int k = 0; k < 4; k++) if (pos[opp][k] == next) pos[opp][k] = -1; } pos[turn][idx] = next; } } else pos[turn][idx] = cur + dice;
      if (pos[turn].every((v) => v == 45)) { gameOver = true; canMove = false; } else { if (dice!=6) turn = 1 - turn; canMove = false; }
    }); syncRoom();
  }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: Color(0xFF0A0E1A), appBar: AppBar(title: Text("ROOM ${widget.roomId} - $myName vs $opponentName", style: TextStyle(fontSize: 11)), actions: [IconButton(icon: Icon(Icons.person_add), onPressed: addFriendFromGame), IconButton(icon: Icon(showChat? Icons.close : Icons.chat), onPressed: () => setState(() => showChat =!showChat))]),
      body: Column(children: [
        ...
        Expanded(child: Center(child: Container(width: s, height: s, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.amber, width: 4)), child: Stack(children: [
          Positioned(left: 10, top: 10, width: box, height: box, child: Container(color: Colors.green.shade100)), Positioned(right: 10, bottom: 10, width: box, height: box, child: Container(color: Colors.red.shade100)),
          Positioned(left: 8, top: box + 8, child: diceNearHome(0, diceGreen)), Positioned(right: 8, bottom: box + 8, child: diceNearHome(1, diceRed)),
          for (int i = 0; i < 40; i++) Positioned(left: s / 2 + s * 0.25 * cos((i / 40) * 2 * pi - pi / 2) - 7, top: s / 2 + s * 0.25 * sin((i / 40) * 2 * pi - pi / 2) - 7, child: Container(width: 14, height: 14, decoration: BoxDecoration(color: safe.contains(i)? Colors.amber : Colors.white, shape: BoxShape.circle, border: Border.all()))),
